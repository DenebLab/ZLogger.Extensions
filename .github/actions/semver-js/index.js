...
const CONFIG_FILE = env("INPUT_CONFIG_FILE", ".abcversion.json");
...
function readSemverFromFile(p) {
  ...
  // Handle JSON format (.abcversion.json)
  if (p.endsWith('.json')) {
    try {
      ...
      if (typeof obj.major === 'number' && typeof obj.minor === 'number') {
        return {
          ...
          minor: String(obj.minor),
          patch: Number(obj.patch || 0)
        };
      }
    } catch (e) {
      throw new Error(`Invalid JSON in config file ${p}: ${e.message}`);
    }
  }
